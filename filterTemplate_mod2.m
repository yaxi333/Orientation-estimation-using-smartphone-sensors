  ...
  try
   ...
   while server.status()  % Repeat while data is available
      ...
      if ~any(isnan(gyr))  % Gyro measurements are available.
          ...
          Rq = eye(4)*0.01; % can be modified based on the data
     ...
     acc = data(1, 2:4)';
      if ~any(isnan(acc))  % Acc measurements are available.
            if  abs(norm(acc)-9.81) < 9.81* 0.25 % acc is not outlier, if 75%|g0| < |acc| < 125%|g0|
      ...
      if ~any(isnan(mag))  % Mag measurements are available.
            ...
            if abs(Lk - norm(mag)) < Lk*0.2   % mag is not outlier, if 80%|L| < |mag| < 120%|L| 
